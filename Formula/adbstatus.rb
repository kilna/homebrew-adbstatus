class Adbstatus < Formula
  ...
  def install
    ...
    # The bin directory already contains Python scripts from pip installation
    # We need to remove them and create our own bash wrapper scripts
    ...
    # Remove the original scripts generated by pip
    rm_f "#{libexec}/bin/adbstatus" if File.exist?("#{libexec}/bin/adbstatus")
    rm_f "#{libexec}/bin/adbstatus-server" if File.exist?("#{libexec}/bin/adbstatus-server")
    rm_f "#{libexec}/bin/adbstatus-monitor" if File.exist?("#{libexec}/bin/adbstatus-monitor")
    
    # Create our bash wrapper scripts that explicitly set PYTHONPATH
    (bin/"adbstatus").write <<~EOS
      #!/bin/bash
      export PYTHONPATH="#{libexec}:$PYTHONPATH"
      #{libexec}/bin/python3 -m adbstatus.core "$@"
    EOS
    
    (bin/"adbstatus-server").write <<~EOS
      #!/bin/bash
      export PYTHONPATH="#{libexec}:$PYTHONPATH"
      #{libexec}/bin/python3 -m adbstatus.server "$@"
    EOS
    
    (bin/"adbstatus-monitor").write <<~EOS
      #!/bin/bash
      export PYTHONPATH="#{libexec}:$PYTHONPATH"
      #{libexec}/bin/python3 -m adbstatus.monitor "$@"
    EOS
